var map = new ol.Map({
    ...
    renderer: 'canvas',
    layers: layersList,
    view: new ol.View({center: ol.proj.fromLonLat([-5101809.0,-2651696.7,-5101351.3,-2651498.3]), // Ajuste para o centro desejado
        zoom: 4
    })
});

//Coordenadas iniciais
map.getView().fit([-5101809.0,-2651696.7,-5101351.3,-2651498.3], map.getSize());
...
var miniMap = new ol.Map({
        target: 'mini-map', // Div onde o mini mapa será renderizado
        renderer: 'canvas',
        layers: layersList2, // Mesmas camadas que o mapa principal
            view: new ol.View({
                center: map.getView().getCenter(),
                zoom: map.getView().getZoom() - 2 // Um zoom menor para o mini mapa
        })
});
...
var layerSwitcher = new ol.control.LayerSwitcher({
	...
	collapseLabel: 'Layers descriptions (legend) »',
...
/* Desabilita as referencias canto direti
//attribution
...
//move controls inside containers, in order
    //zoom
    var zoomControl = document.getElementsByClassName('ol-zoom')[1];
    if (zoomControl) {
...
*/
    //Definir barra de escale
var scaleLineControl = new ol.control.ScaleLine({
    units: 'metric' // ou 'imperial' para usar unidades imperiais
});

map.addControl(scaleLineControl);


//Definir parametros do mini mapa
miniMap.getInteractions().forEach(function(interaction) {
...
// Função para sincronizar o mini mapa com o principal
function syncminiMap() {
    miniMap.getView().setCenter(map.getView().getCenter());
    miniMap.getView().setZoom(map.getView().getZoom() - 8); // Ajuste o zoom como preferir
}
